...
class HFRRes3D:
    ...
    # 2) vertical adjacency between scale l and l+1 (UPDATED VERSION)
        for level in range(self.n_levels-1):
            k_l   = self.cells_per_level[level]
            k_lp1 = self.cells_per_level[level+1]
            labels_l   = partitions[level]
        ...
        # Parent-to-child (coarse-to-fine)
            Xvert = np.zeros((k_l, k_lp1))
            for t in range(N):
                i = labels_l[t]
                j = labels_lp1[t]
        ...
        # Child-to-parent (fine-to-coarse) - calculated separately
            Yvert = np.zeros((k_lp1, k_l))
            for t in range(N):
                j = labels_lp1[t]
                i = labels_l[t]
                Yvert[j,i] += 1
            row_sum = Yvert.sum(axis=1, keepdims=True)
            row_sum[row_sum==0.0] = 1.0
            Yvert /= row_sum
        
        # Place both in matrix
            off_l   = offsets[level]
            off_lp1 = offsets[level+1]
            A[off_l:off_l+k_l, off_lp1:off_lp1+k_lp1] = Xvert
            A[off_lp1:off_lp1+k_lp1, off_l:off_l+k_l] = Yvert
